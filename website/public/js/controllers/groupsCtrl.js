...
habitrpg.controller("GroupsCtrl", ['$scope', '$rootScope', 'Shared', 'Groups', '$http', '$q', 'User', 'Members', '$state', 'Notification',
  ...
  .controller('ChatCtrl', ['$scope', 'Groups', 'User', '$http', 'ApiUrl', 'Notification', 'Members', '$rootScope', 'Analytics', function($scope, Groups, User, $http, ApiUrl, Notification, Members, $rootScope, Analytics){
    ...
    $scope.postChat = function(group, message){
      ...
      Groups.Group.postChat({gid: group._id, message:message, previousMsg: previousMsg}, undefined, function(data){
        ...
        if (group.privacy == 'public'){
          Analytics.track('Group Chat',{'groupType':group.type,'privacy':group.privacy,'groupName':group.name,'message':message})
        } else {
          Analytics.track('Group Chat',{'groupType':group.type,'privacy':group.privacy})
        }
      }, function(err){
        $scope._sending = false;
      });
    ...
    function($scope, Groups, User, Challenges, $rootScope, $state, $location, $compile, Analytics) {
      ...
      $scope.create = function(group){
        ...
        if (confirm(window.env.t('confirmGuild'))) {
          group.$save(function(saved){
            if (saved.privacy == 'public') {Analytics.track('Join Group',{'owner':true,'groupType':'guild','privacy':saved.privacy,'groupName':saved.name})}
            else {Analytics.track('Join Group',{'owner':true,'groupType':'guild','privacy':saved.privacy})}
      ...
      $scope.join = function(group){
        ...
        group.$join(function(joined){
          if (joined.privacy == 'public') {Analytics.track('Join Group',{'owner':false,'groupType':'guild','privacy':joined.privacy,'groupName':joined.name})}
          else {Analytics.track('Join Group',{'owner':false,'groupType':'guild','privacy':joined.privacy})}
  ...
  .controller("PartyCtrl", ['$rootScope','$scope', 'Groups', 'User', 'Challenges', '$state', '$compile', 'Analytics',
    function($rootScope,$scope, Groups, User, Challenges, $state, $compile, Analytics) {
      ...
      $scope.create = function(group){
        group.$save(function(){
          Analytics.track('Join Group',{'owner':true,'groupType':'party','privacy':'private'});
      ...
      $scope.join = function(party){
        ...
        group.$join(function(){
          Analytics.track('Join Group',{'owner':false,'groupType':'party','privacy':'private'});
          $rootScope.hardRedirect('/#/options/groups/party');
        });
      }
